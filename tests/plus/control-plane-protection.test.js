...
import { defaultPlusControlPaths } from '../../plus/control/control-paths.js';
...
test('Plus control-plane directory is created owner-only and always protected from broad workspace mounts', async () => {
  ...
  const plusPaths = defaultPlusControlPaths(home);

  const protectedPaths = await defaultProtectedPaths({ home, configPath, platform: 'darwin' });

  assert.equal(plusPaths.root, plusControl);
  assert.equal(plusPaths.hostRegistry, path.join(plusControl, 'host-registry.json'));
