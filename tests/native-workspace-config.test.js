...
test('workspace config defaults network by mode and keeps Git publication opt-in', () => {
  const project = normalizeWorkspaceConfig({ version: 1, hostRoot: '/tmp/project', mode: 'project' }, { platform: 'linux' });
  assert.equal(project.networkEnabled, true);
  assert.equal(project.gitPublicationEnabled, false);

  const advanced = normalizeWorkspaceConfig({ version: 1, hostRoot: '/tmp/all', mode: 'advanced' }, { platform: 'linux' });
...
test('workspace config rejects unknown keys, relative roots, and literal macOS root', () => {
  ...
  assert.equal(normalizeWorkspaceConfig({
    version: 1,
    hostRoot: '/',
    mode: 'advanced',
  }, { platform: 'linux' }).hostRoot, '/');
});
