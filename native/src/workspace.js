  ...
  return {
    ...
    async openWorkspace(requestedRoot) {
      ...
      return {
        ...
        root: NATIVE_WORKSPACE_ROOT,
        mode: 'checkout',
        instruction: 'Workspace opened. Before the first modification for a task: locate the target project, read its AGENTS.md, apply the WebMCP resume protocol, create or refresh the workspace-owned semantic checkpoint under /workspace/.webmcp/resumes/, and update /workspace/CHATGPT-RESUME.md as a pointer only. During recovery, read the referenced checkpoint and inspect Git/filesystem state; actual Git/filesystem state is authoritative.',
      };
    },
