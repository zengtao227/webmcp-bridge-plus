...
export async function buildNativeContainerRun({
  ...
  const canonicalRoot = await canonicalizeHostRoot(normalized.hostRoot, { platform });
  ...
  const effectiveProtectedPaths = normalized.gitPublicationEnabled
    ...
    : protectedPaths;
  const maskPlan = await buildControlPlaneMaskPlan({
    hostRoot: canonicalRoot,
    protectedPaths: effectiveProtectedPaths,
  });

  const policyDigest = sha256Text(JSON.stringify({
    version: 1,
    image,
    hostRoot: canonicalRoot,
